...
criteria = FastSpeechLoss().to(device)
...
while global_step < num_steps:
    ...
    for batch in dataloader:
        ...
        loss = criteria(mel_pred, mel_target,duration_pred, duration_target)
        ...
        if global_step % log_interval == 0:
            ...
            print(f'Step [{global_step}/{num_steps}], Loss: {avg_loss:.4f}')
            total_loss = 0
    
        if global_step % save_interval == 0:
            torch.save(model.module.state_dict() if torch.cuda.device_count() > 1 else model.state_dict(), f'fastspeech_step_{global_step}.pth')
