...
def ipa(text: str) -> str:
    ...
    phonetic = re.sub(
        r"(" + palatalizable + ")([ː]?)([ˈ]?)i", r"\1ʲ\2\3i", phonetic)
    ...
    phonetic = re.sub(
        r"([^ɑɛiɪuɔ]+)ʲ([^ɑɛiɪuɔ]+)ʲ([^ɑɛiɪuɔ]+)ʲ", r"\1\2ʲ\3ʲ", phonetic)
    ...
    phonetic = re.sub(
        r"ɔ([bdzʒɡɦmnlrpftskxʲʃ͡]+)ˈ([uiʊ]+)", r"o\1ˈ\2", phonetic)
    ...
    phonetic = re.sub(
        r"^j(" + re.sub(r"ʋ", r"", consonant) + "+)", r"i̯\1", phonetic)
    ...
    pattern = (
    r'ʒt͡s|ʒd͡ʒ|sʲː|sʲt͡sʲ|t͡sʲː|zʲt͡sʲ|d͡zʲ|t͡sʲː|d͡ʒ|d͡z|d͡zs|d͡ʒʃ|d͡ʒt͡ʃ|d͡zt͡s|t͡s|t͡ʃʃ|t͡ʃː|t͡sː|'
    r'ʃt͡ʃ|'
    r'[ɑɛiɪuɔɐoʊe]̯?ʲ?ˈ?|'
    r'[bdzʒɡɦmnlrpftskxʃʋ]ʲ?ˈ?|'
    r'ʲ|ˈ'
)
    phonetic_list = re.findall(pattern, phonetic)
